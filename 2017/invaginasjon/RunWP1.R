getwd()
# setwd("F:/Forskningsprosjekter/PDB 1472 - Forekomst av rotavir_/Forskningsfiler/LIVF/Invaginasjon")
...
unlink(file.path(RAWmisc::PROJ$SHARED,lubridate::today()), recursive=TRUE, force=TRUE)
dir.create(file.path(RAWmisc::PROJ$SHARED,lubridate::today()))
...
openxlsx::write.xlsx(GetResults(data,useMean=T),file=file.path(RAWmisc::PROJ$SHARED,lubridate::today(),"mean_estimates.xlsx"))
...
saveRDS(estimates,file=file.path(RAWmisc::PROJ$BAKED,"estimates.RDS"))

rmarkdown::render("Notes.Rmd", output_dir = file.path(RAWmisc::PROJ$SHARED,lubridate::today()))
