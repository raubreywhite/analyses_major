...
## XX. Total number of working Radiologists in Palestine

### Results BY CW00

```{r message=FALSE, warning=FALSE}
d <- dWorkers[isProf_Radiologist==1]
d[,numerator:=1]
...
```{r message=FALSE, warning=FALSE}
d <- dWorkers[BOTH_isProf_Radiologist==1]
d[,numerator:=1]
...
## XX. Total number of working Lab Techs in Palestine

### Results BY CW00

```{r message=FALSE, warning=FALSE}
d <- dWorkers[isProf_LabTech==1]
d[,numerator:=1]
...
```{r message=FALSE, warning=FALSE}
d <- dWorkers[BOTH_isProf_LabTech==1]
d[,numerator:=1]
...
## 28/30. Percentage of health workers from the Ministry of Health who dual practice IN HOSPITALS
...
## 28/30. Percentage of health workers from the Ministry of Health who dual practice IN HEALTH CENTERS
...
for(i in 1:nrow(stack)){
  ...
  if(stack[i]$dist=="isPalestine" & stack[i]$sector=="All sectors"){
    denominator <- length(unique(dWorkers[isProf_Healthworker==TRUE]$HR01))
  } else {
