...
jordanvalley <- dFacilities[RE01==1,
                           c("IDH00","IDH01","IDH02","IDH03","IS06","IS04","facilitySector","RE01","RE02")]
...
```{r}
d <- dFacilities[RE01==1,.(N=.N),by=.(facilitySector,IS06)]
setorder(d,facilitySector,IS06)
print(d)
```
...
dED03 <- data.table(rbind(d1,d2,d3[names(d1)],d4[names(d1)]))
...
```{r}
d <- copy(dWorkers)
d[,age:=floor(as.numeric(difftime("2017-01-01",HR03,units="days"))/365.25)]
...
whereDoTheyWork <- dFacilities[,c("IDH00","RE01","RE02")]
whereDoTheyWork[RE02==1,worksInAreaC:=1]
whereDoTheyWork[RE01==1,worksInJordanValley:=1]
whereDoTheyWork[,RE01:=NULL]
...
dWorkers[,age:=floor(as.numeric(difftime("2017-01-01",HR03,units="days"))/365.25)]
