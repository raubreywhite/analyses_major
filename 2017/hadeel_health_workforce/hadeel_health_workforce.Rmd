...
# missing CW00
updatedCW00 <- data.table(readxl::read_excel(file.path(RAWmisc::PROJ$RAW,
                                    "data_richard",
                                    "missing_CW00.xlsx")))
...
updatedCW00x <- data.table(readxl::read_excel(file.path(RAWmisc::PROJ$RAW,
                                    "data_richard",
...
updatedCW00 <- unique(rbind(updatedCW00,updatedCW00x))

# adding a list of GPs
listOfGPs <- data.table(readxl::read_excel(file.path(RAWmisc::PROJ$RAW,
                                    "data_richard",
...
d4 <- readxl::read_excel(file.path(RAWmisc::PROJ$RAW,
                                    "Workers records to be removed",
                                    "to remove.xlsx"))
dxxx <- d5 <- readxl::read_excel(file.path(RAWmisc::PROJ$RAW,
                                    "Workers records to be removed",
...
dxxx2 <- d5 <- haven::read_spss(file.path(RAWmisc::PROJ$RAW,
                                    "New 8,000 records for data importing-received from Fayez",
                                    "worker_main (6).sav"))
...
if(EXTRA_8000){
  dWorkers <- data.table(rbind(d1,d2,d3,d4,d5,dUNRWA))
} else {
  dWorkers <- data.table(rbind(d1,d2,d3,d4,dUNRWA))
}
...
correctAges <- data.table(readxl::read_excel(file.path(RAWmisc::PROJ$RAW,"data_richard","conflicing_ages.xlsx")))
...
openxlsx::write.xlsx(dWorkers[numAgesNotMissing>1,c("HR01","HR03")],file.path(RAWmisc::PROJ$SHARED_TODAY,"conflicing_ages.xlsx"))
...
# GENDERS FIX 1
updatedGender <- data.table(readxl::read_excel(file.path(RAWmisc::PROJ$RAW,
                                    "data_richard",
...
# POPULATION
pop <- data.table(readxl::read_excel(file.path(RAWmisc::PROJ$RAW,"data_richard/pop_estimates_2017.xlsx")))
setnames(pop,c("districtName","pop","districtCode","area"))

popunrwa <- data.table(readxl::read_excel(file.path(RAWmisc::PROJ$RAW,"data_richard/pop_estimates_2017_unrwa.xlsx")))
...
cat(dFacilities$IDH00[duplicated(dFacilities$IDH00)],file=file.path(RAWmisc::PROJ$SHARED_TODAY,"DUPLICATED_FACILITY_IDH00_NUMBERS.txt"))

# SAVING FILES
openxlsx::write.xlsx(dFacilities,file.path(RAWmisc::PROJ$SHARED_TODAY,"dFacilities.xlsx"))
openxlsx::write.xlsx(dWorkers,file.path(RAWmisc::PROJ$SHARED_TODAY,"dWorkers.xlsx"))
```
...
## MoH West Bank official list

I have also included a list on our shared file called MoH West Bank official list, could you please cross-check our MoH West Bank records with the MoH official list by ID and let me know if we have any still missing in our records. 

```{r, warning=FALSE}
dMaster <- merge(dWorkers, dFacilities[IDH03 %in% dLocation[isGaza==0]$localityCode,c("IDH00","facilitySector")], by="IDH00",all.x=T)
nrow(dWorkers)
nrow(dMaster)
#dMaster[HR01=="910435585"]

dMOHMaster <- data.table(readxl::read_excel(file.path(RAWmisc::PROJ$RAW,"data_richard","MoH West Bank Official list of employees.xlsx")))
dMOH <- unique(na.omit(data.table(HR01=as.numeric(dMOHMaster[["ID"]]))))

dMOH_From_System <- unique(dMaster[facilitySector=="Ministry of Health",c("HR01")])
...
openxlsx::write.xlsx(final,file.path(RAWmisc::PROJ$SHARED_TODAY,"VALIDATING MoH West Bank official list.xlsx"))

openxlsx::write.xlsx(dWorkers[HR01 %in% final$SYSTEM_ONLY,NAMES_OF_WORKERS_FILE,with=F],file.path(RAWmisc::PROJ$SHARED_TODAY,"VALIDATING MoH West Bank official list_DATASET_THAT_EXISTS_IN_SYSTEM_BUT_NOT_MOH_LIST.xlsx"))

openxlsx::write.xlsx(dMOHMaster[ID %in% final$MOH_ONLY],file.path(RAWmisc::PROJ$SHARED_TODAY,"VALIDATING MoH West Bank official list_DATASET_THAT_EXISTS_IN_MOH_LIST_BUT_NOT_SYSTEM.xlsx"))

sum(dMOHMaster$ID %in% final$BOTH)
sum(dMOHMaster$ID %in% final$MOH_ONLY)
sum(dMOHMaster[ID %in% final$MOH_ONLY]$ID %in% dxxx$HR01)
sum(dMOHMaster[ID %in% final$MOH_ONLY]$ID %in% dxxx2$HR01)
sum(dMOHMaster[ID %in% final$MOH_ONLY]$ID %in% dWorkers$HR01)


#dWorkers[HR01 %in% dMOHMaster[ID %in% final$MOH_ONLY & ID %in% dxxx2$HR01]]
...
#problematic <- dWorkers[HR01 %in% final$MOH_ONLY]
...
for(i in unique(dMaster$facilitySector)){
  if(is.na(i)){
    d <- dMaster[is.na(facilitySector) & BOTH_isProf_Doctor==1]
  } else {
    d <- dMaster[facilitySector==i & BOTH_isProf_Doctor==1]
  }
  d[,numerator:=as.numeric(NA)]
  d[!is.na(SK02),numerator:=0]
  d[SK02==1,numerator:=1]
...
for(i in c("isPalestine","isWB","isGaza")){
  d <- dWorkers[get(i)==1 & BOTH_isProf_GP==1,.(
    numerator=1
  ),by=.(
    HR01
  )]
...
for(i in unique(dMaster$facilitySector)){
  if(is.na(i)){
    d <- dMaster[is.na(facilitySector) & isProf_GP==1]
  } else {
    d <- dMaster[facilitySector==i & isProf_GP==1]
  }
...
for(i in unique(dMaster$facilitySector)){
  if(is.na(i)){
    d <- dMaster[is.na(facilitySector) & BOTH_isProf_GP==1]
  } else {
    d <- dMaster[facilitySector==i & BOTH_isProf_GP==1]
  }
...
for(j in c("isPalestine","isWB","isGaza")) for(i in unique(dMaster$facilitySector)) {
  if(is.na(i)){
    d <- dMaster[is.na(facilitySector) & BOTH_isProf_GP==1 & get(j)==1]
  } else {
    d <- dMaster[facilitySector==i & BOTH_isProf_GP==1 & get(j)==1]
  }
...
for(i in c("isPalestine","isWB","isGaza")){
  ...
  openxlsx::write.xlsx(d,
                       file.path(RAWmisc::PROJ$SHARED_TODAY,sprintf("CW00_%s_Total number of specialized doctors by specialty by sector in Palestine, West Bank, Gaza.xlsx",i)))
}
```

### Results BY CW00 AND CW06

GO TO: "CW00_AND_CW06_Total number of specialized doctors by specialty by sector in Palestine, West Bank, Gaza.xlsx"

```{r message=FALSE, warning=FALSE}
for(i in c("isPalestine","isWB","isGaza")){
  d <- dMaster[get(i)==1]
  d <- d[!is.na(BOTHdoctor_specialization),.(
    numerator=1
  ),by=.(
    HR01,
    facilitySector,
  ...
  setorder(d,facilitySector,doctor_specialization)
  openxlsx::write.xlsx(d,
                       file.path(RAWmisc::PROJ$SHARED_TODAY,sprintf("CW00_AND_CW06_%s_Total number of specialized doctors by specialty by sector in Palestine, West Bank, Gaza.xlsx",i)))
}
```

## Age distribution of General Practitioners in Palestine, West Bank, Gaza

### BY CW00/ BY CW00 and CW06 ("BOTH")
...
for(prof in c("isProf_GP","BOTH_isProf_GP")) for(area in c(
    "isPalestine",
    "isGaza",
    "isWB"
  )){
...
for(prof in c("isProf_GP","BOTH_isProf_GP")) for(sector in unique(dMaster$facilitySector)) for(area in c(
    "isPalestine",
    "isGaza",
    "isWB"
  )){
...
for(i in c("isPalestine","isWB","isGaza")){
  d <- dWorkers[get(i)==1 & BOTH_isProf_FamilyPhysician==1,.(
    numerator=1
  ),by=.(
    HR01
  )]
...
for(i in c("isPalestine","isWB","isGaza")) for(j in unique(dMaster$facilitySector)){
  if(is.na(j)) next
  d <- dMaster[get(i)==1 & facilitySector==j & BOTH_isProf_FamilyPhysician==1,.(
    numerator=1
  ),by=.(
    HR01
  )]
...
for(i in unique(dMaster$region)){
  if(is.na(i)) next
  d <- dMaster[region==i & BOTH_isProf_GP==1,.(
    numerator=1
  ),by=.(
    HR01
  )]
...
for(location in c("isPalestine","isWB","isGaza")){
  ...
  d <- dMaster[BOTH_isProf_Doctor==1 & get(location)==1 & facilitySector=="UNRWA"]
  d[,numerator:=1]
...
for(prof in professionVarsBOTH) for(location in c("isPalestine","isWB","isGaza")){
  cat("\n\n********",prof,"  ",location,"**\n\n")
  if(location=="isPalestine"){
    usePopulation <- popunrwa[districtName=="Palestine population"]$pop
  } else if(location=="isWB"){
...
openxlsx::write.xlsx(d,file.path(RAWmisc::PROJ$SHARED_TODAY,"hospital_discrepancies.xlsx"))
```
...
## WHAT COUNTRY DO GPs GRADUATE FROM CW00 AND CW06

```{r, warning=FALSE}
d <- dWorkers[BOTH_isProf_GP==1]
nrow(d)
d <- merge(d,dED03,by="HR01",all.x=T)
nrow(d)
...
```{r, warning=FALSE}
workerLabels <- data.table(readxl::read_excel(file.path(RAWmisc::PROJ$RAW,
...
### sector = Ministry of Health  + IS06 = 4 BY CW00 AND CW06
...
d <- d[,.(N=.N),by=.(gazaWBEJ,facilitySector,IS06)]
setorder(d,gazaWBEJ,facilitySector,IS06)
openxlsx::write.xlsx(d,file.path(RAWmisc::PROJ$SHARED_TODAY,"clinical_training_facility_is06.xlsx"))
```
...
openxlsx::write.xlsx(d,file.path(RAWmisc::PROJ$SHARED_TODAY,"clinical_training_facility_is06_TR02.xlsx"))
```
...
d <- d[,.(N=.N),by=.(gazaWBEJ,facilitySector,IS06)]
setorder(d,gazaWBEJ,facilitySector,IS06)
openxlsx::write.xlsx(d,file.path(RAWmisc::PROJ$SHARED_TODAY,"clinical_internship_facility_is06.xlsx"))
```
...
openxlsx::write.xlsx(d,file.path(RAWmisc::PROJ$SHARED_TODAY,"clinical_internship_facility_is06_TR06.xlsx"))
```
...
for(location in c("isPalestine","isWB","isGaza")){
  ...
  d <- dWorkers[BOTH_isProf_Radiologist==1 & get(location)==1]
  d[,numerator:=1]
...
for(location in c("isPalestine","isWB","isGaza")){
  ...
  d <- dWorkers[BOTH_isProf_LabTech==1 & get(location)==1]
  d[,numerator:=1]
...
for(location in c("isPalestine","isWB","isGaza")){
  ...
  d <- dWorkers[BOTH_isProf_Doctor==1 & get(location)==1]
  d[,numerator:=1]
...
for(location in c("isPalestine","isWB","isGaza")){
  ...
  d <- dWorkers[BOTH_isProf_Specializeddoctor==1 & get(location)==1]
  d[,numerator:=1]
...
for(location in c("isPalestine","isWB","isGaza")){
  ...
  d <- dWorkers[BOTH_isProf_Dentist==1 & get(location)==1]
  d[,numerator:=1]
...
for(location in c("isPalestine","isWB","isGaza")){
  ...
  d <- dWorkers[BOTH_isProf_Pharmacist==1 & get(location)==1]
  d[,numerator:=1]
...
for(location in c("isPalestine","isWB","isGaza")){
  ...
  d <- dWorkers[BOTH_isProf_Nurse==1 & get(location)==1]
  d[,numerator:=1]
...
for(location in c("isPalestine","isWB","isGaza")){
  ...
  d <- dWorkers[BOTH_isProf_Midwife==1 & get(location)==1]
  d[,numerator:=1]
...
for(location in c("isPalestine","isWB","isGaza")){
  ...
  d <- dWorkers[BOTH_isProf_Alliedhealthsciences==1 & get(location)==1]
  d[,numerator:=1]
...
for(location in c("isPalestine","isWB","isGaza")){
  ...
  d <- dWorkers[BOTH_isProf_Psychologistcounselorstherap==1 & get(location)==1]
  d[,numerator:=1]
...
for(location in c("isPalestine","isWB","isGaza")){
  ...
  d <- dWorkers[BOTH_isProf_Psychiatrist==1 & get(location)==1]
  d[,numerator:=1]
...
for(location in c("isPalestine","isWB","isGaza")){
  ...
  d <- dWorkers[BOTH_isProf_AlternativeMedicine==1 & get(location)==1]
  d[,numerator:=1]
...
for(location in c("isPalestine","isWB","isGaza")){
  ...
  d <- dWorkers[(BOTH_isProf_Doctor==1 | BOTH_isProf_Nurse==1 | BOTH_isProf_Midwife==1) & get(location)==1]
  d[,numerator:=1]
...
for(location in c("isPalestine","isWB","isGaza")){
  ...
  d <- dWorkers[(BOTH_isProf_Administrativesupportstaff==1) & get(location)==1]
  d[,numerator:=1]
...
for(i in unique(dMaster$facilitySector)){
  for(location in c("isPalestine","isWB","isGaza")){
    ...
    d <- dMaster[facilitySector==i & (BOTH_isProf_Healthworker==1 | BOTH_isProf_Administrativesupportstaff==1) & get(location)==1]
...
for(i in specializations){
  for(location in c("isPalestine","isWB","isGaza")){
    ...
    d <- dMaster[BOTHdoctor_specialization==i & get(location)==1]
...
for(i in c("All",unique(dMaster$facilitySector))){
  for(location in c("isPalestine","isWB","isGaza")){
    ...
    d <- dMaster[(BOTH_isProf_Doctor==1 | BOTH_isProf_Nurse==1) & get(location)==1]
    if(i!="All") d <- d[facilitySector==i]
...
for(i in c("All",unique(dMaster$facilitySector))){
  for(location in c("isPalestine","isWB","isGaza")){
    ...
    d <- dMaster[(BOTH_isProf_Administrativesupportstaff==1 | BOTH_isProf_Healthworker==1) & get(location)==1]
    if(i!="All") d <- d[facilitySector==i]
    
    d[,numerator:=BOTH_isProf_Administrativesupportstaff]
    d[,denominator:=BOTH_isProf_Healthworker]
...
### Results BY CW00 AND CW06 IN HOSPITALS
...
for(i in professionVarsBOTH){
  for(location in c("isPalestine","isWB","isGaza")){
    ...
    d <- dMaster[get(i)==1 & get(location)]
...
### BY SPECIALIZED DOCTOR BY CW00 IN HOSPITALS
...
### BY SPECIALIZED DOCTOR BY CW00 AND CW06 IN HOSPITALS
...
### Results BY CW00 IN HEALTH CENTERS
...
### Results BY CW00 AND CW06 IN HEALTH CENTERS
...
### BY SPECIALIZED DOCTOR BY CW00 IN HEALTH CENTERS
...
### BY SPECIALIZED DOCTOR BY CW00 AND CW06 IN HEALTH CENTERS
...
for(location in c("isPalestine","isWB","isGaza")){
  ...
  d <- merge(dWorkers, dFacilities[,c("IDH00","IS06")], by="IDH00")
  d <- d[BOTH_isProf_Doctor==1 & get(location)==1]
  d[!is.na(IS06),numerator:=0]
  d[IS06==9,numerator:=1]
...
for(location in c("isPalestine","isWB","isGaza")){
  ...
  d <- merge(dWorkers, dFacilities[,c("IDH00","IS06")], by="IDH00")
  d <- d[BOTH_isProf_Nurse==1 & get(location)==1]
  d[!is.na(IS06),numerator:=0]
  d[IS06==9,numerator:=1]
...
for(location in c("isPalestine","isWB","isGaza")){
  ...
  d <- merge(dWorkers, dFacilities[,c("IDH00","IS06","facilitySector")], by="IDH00")
  d <- d[BOTH_isProf_Specializeddoctor==1 & get(location)==1]
  d[!is.na(IS06),numerator:=0]
  d[IS06 %in% c(1,2),numerator:=1]
  d[facilitySector=="Private",numerator:=1]
...
## BY CW00 FOR MINISTRY OF HEALTH AND IS06==9
...
## BY CW00 AND CW06 FOR MINISTRY OF HEALTH AND IS06==9
...
### STRATIFIED BY SPECIALIZED DOCTOR BY CW00 FOR MINISTRY OF HEALTH AND IS06==9
...
### STRATIFIED BY SPECIALIZED DOCTOR BY CW00 AND CW06 FOR MINISTRY OF HEALTH AND IS06==9
...
for(prof in specializations) for(area in c(
    ...
    d <- dWorkers[BOTHdoctor_specialization==prof & get(area)==1 & IDH00 %in% keepIDH00]
    d[,numerator:=1]
...
## BY CW00 FOR MINISTRY OF HEALTH AND IS06==4
...
## BY CW00 AND CW06 FOR MINISTRY OF HEALTH AND IS06==4
...
### STRATIFIED BY SPECIALIZED DOCTOR BY CW00 FOR MINISTRY OF HEALTH AND IS06==4
...
### STRATIFIED BY SPECIALIZED DOCTOR BY CW00 AND CW06 FOR MINISTRY OF HEALTH AND IS06==4
...
### IS06==9 & MINISTRY OF HEALTH BY CW00 AND CW06
...
### IS06==4 & MINISTRY OF HEALTH BY CW00 AND CW06
...
for(i in 1:nrow(stack)){
  ...
  allFemalesInPalestine <- length(unique(dWorkers[BOTHdoctor_specialization==s$prof & isFemale==TRUE]$HR01))
  d[,numerator:=1]
...
for(i in unique(dMaster$facilitySector)){
  d <- dMaster[facilitySector==i & BOTH_isProf_Healthworker==1]
  allFemalesInPalestine <- length(unique(dWorkers[BOTH_isProf_Healthworker==1 & isFemale==TRUE]$HR01))
  d[,numerator:=1]
...
```{r, warning=FALSE}
stack <- data.table(expand.grid(prof=professionVarsBOTH,dist=districtNames,
  sector=c("All sectors",unique(dFacilities$facilitySector)),stringsAsFactors = F))
...
## CW00 Age distribution of specialist doctors by specialisation, by Pal/WB/Gaza
...
## CW00 AND CW06 Age distribution of specialist doctors by specialisation, by Pal/WB/Gaza

```{r, warning=FALSE}

specializations <- sort(unique(dWorkers$BOTHdoctor_specialization))
stack <- na.omit(data.table(expand.grid(dist=districtNames,prof=specializations,stringsAsFactors = F)))
...
## CW00 Gender distribution of specialist doctors by specialisation, by Pal/WB/Gaza/District
...
## CW00 AND CW06 Gender distribution of specialist doctors by specialisation, by Pal/WB/Gaza/District
...
## CW00 Geographical distribution of specialist doctors by specialisation
...
## CW00 AND CW06 Geographical distribution of specialist doctors by specialisation
...
for(i in 1:nrow(stack)){
  ...
  d <- dWorkers[BOTHdoctor_specialization==s$prof]
  temp <- data.frame(area="00PALESTINE (no duplicates)",num=length(unique(d$HR01)))
  res[["00PALESTINE (no duplicates)"]] <- temp
  
  for(j in areas){
    d <- dWorkers[BOTHdoctor_specialization==s$prof & get(j)==TRUE]
    temp <- data.frame(area=j,num=length(unique(d$HR01)))
    res[[j]] <- temp
  }
  d <- rbindlist(res)
...
dED03 <- masterdED03[ED03A==3]
d <- merge(dWorkers[ED01==3],dED03,by="HR01")
print(nrow(d[BOTH_isProf_Nurse==TRUE]))
```
...
for(i in c("All",unique(dMaster$facilitySector))){
  for(location in c("isPalestine","isWB","isGaza")){
    ...
    d <- dMaster[(BOTH_isProf_Specializeddoctor==1 | BOTH_isProf_GP==1) & get(location)==1]
    if(i!="All") d <- d[facilitySector==i]
    
    d[,numerator:=BOTH_isProf_Specializeddoctor]
    d[,denominator:=BOTH_isProf_GP]
    ...
    cat(sprintf("***** %s %s *****\n",i,location))
    cat(sprintf("BOTH_isProf_Specializeddoctor: %s\n",sum(d$numerator)))
    cat(sprintf("BOTH_isProf_GP: %s\n",sum(d$denominator)))
    cat(sprintf("Ratio: %s\n",sum(d$numerator)/sum(d$denominator)))
    cat("\n\n")
  }
}
...
for(location in c("isPalestine","isWB","isGaza")){
  ...
  d <- dWorkers[BOTH_isProf_PharmacyAssistant==1 & get(location)==1]
  d[,numerator:=1]
...
## WHAT COUNTRY DO NURSES GRADUATE FROM CW00 AND CW06
...
## WHAT COUNTRY DO dentists GRADUATE FROM CW00 AND CW06
...
## WHAT COUNTRY DO pharmacists GRADUATE FROM CW00 AND CW06
