...
dMOHMaster <- data.table(readxl::read_excel(file.path(RAWmisc::PROJ$RAW,"Fixed excels - to be included in calculations","2018 MoH WB employees with DOB - Richard.xlsx")))
...
## checking to see if date of births are different
## TODO MORE HERE
...
## end
sum(dMOHMaster$HR01 %in% final$BOTH)
sum(dMOHMaster$HR01 %in% final$MOH_ONLY)
sum(dMOHMaster[HR01 %in% final$MOH_ONLY]$HR01 %in% dxxx$HR01)
sum(dMOHMaster[HR01 %in% final$MOH_ONLY]$HR01 %in% dxxx2$HR01)
sum(dMOHMaster[HR01 %in% final$MOH_ONLY]$HR01 %in% dWorkers$HR01)
