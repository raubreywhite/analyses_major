...
dir.create(fs::path(SHARED_TODAY,"crosscheck"))
...
dMaster <- merge(dWorkers, unique(dFacilities[,c("IDH00","facilitySector")]), by="IDH00",all.x=T)
...
dMaster <- merge(dWorkers, unique(dFacilities[facilitySector=="Ministry of Health" & IDH03 %in% dLocation[isGaza==0]$localityCode,c("IDH00","facilitySector")]), by="IDH00")
nrow(dWorkers)
nrow(dMaster)
...
```{r}
# crosscheck with system dir.create(fs::path(SHARED_TODAY,"crosscheck"))
fac <- fread(fs::path(org::PROJ$RAW,"HRH Files","Facilities_27_2_2019.csv"),header=F)
wor1 <- fread(fs::path(org::PROJ$RAW,"HRH Files","workers information27_2_2019.csv"))
wor2 <- fread(fs::path(org::PROJ$RAW,"HRH Files","workers information_with_Experince27_2_2019.csv"))
...
dFac <- merge(
  dFacilities,
  unique(dLocation[,c("localityCode","localityName")]),
  by.x=c("IDH03"),
  by.y=c("localityCode")
  )
nrow(dFac)
...
dFac <- merge(
  dFac,
  unique(dLocation[district!="East Jerusalem (J1)",c("districtCode","district")]),
  by.x=c("IDH01"),
  by.y=c("districtCode")
  )
nrow(dFac)
...
dFac[,district:=only_letters(district)]
fac[,V2:=only_letters(V2)]

dFac[district=="toubas",district:="tubas"]
dFac[district=="toulkarem",district:="tulkarm"]
dFac[district=="qalqilya",district:="qalqiliya"]
dFac[district=="ramallah",district:="ramallahandalbireh"]
dFac[district=="gazacity",district:="gaza"]
dFac[district=="deralbalah",district:="deirelbalah"]
dFac[district=="khanyounis",district:="khanyunis"]

unique(dFac$district)[!unique(dFac$district) %in% unique(fac$V2)]
unique(fac$V2)[!unique(fac$V2) %in% unique(dFac$district)]


dFac[,localityName:=only_letters(localityName)]
fac[,V3:=only_letters(V3)]

dFac[,from_richard:=TRUE]
dFac[,richard_id1:=1:.N]
fac[,from_system:=TRUE]
fac[,richard_id2:=1:.N]

xf <- merge(
  dFac,
  fac,
  by.x=c("IDH02","district"),
  by.y=c("V1","V2"),
  all=T)

xf <- xf[is.na(from_richard) | is.na(from_system)]
nrow(xf)

#idh02
if(length(na.omit(xf$richard_id1))>0){
  openxlsx::write.xlsx(
    dFac[richard_id1 %in% na.omit(xf$richard_id1)],
    fs::path(SHARED_TODAY,"crosscheck","fac_missing_in_system.xlsx")
  )
}
if(length(na.omit(xf$richard_id2))>0){
  openxlsx::write.xlsx(
    fac[richard_id2 %in% na.omit(xf$richard_id2)],
    fs::path(SHARED_TODAY,"crosscheck","fac_extra_in_system.xlsx")
  )
}


#workers HR01
dMaster <- merge(dWorkers, unique(dFacilities[,c("IDH00","IDH01","IDH02","IDH03")]), by="IDH00",all.x=T)
nrow(dMaster)
nrow(dWorkers)
dMaster[,`Jobs Titles Groups`:="Undefined"]
dMaster[is.na(CW00),`Jobs Titles Groups`:="Missing"]
dMaster[CW00isProf_Administrativesupportstaff==TRUE,`Jobs Titles Groups`:="Administrative & support Staff"]
dMaster[CW00isProf_Specializeddoctor==TRUE,`Jobs Titles Groups`:="Specialized doctor"]
dMaster[CW00isProf_Alliedhealthsciences==TRUE,`Jobs Titles Groups`:="Allied health sciences"]
dMaster[CW00isProf_Nurse==TRUE,`Jobs Titles Groups`:="Nurse"]
dMaster[CW00isProf_AlternativeMedicine==TRUE,`Jobs Titles Groups`:="Alternative Medicine"]
dMaster[CW00isProf_Dentist==TRUE,`Jobs Titles Groups`:="Dentist"]
dMaster[CW00isProf_GP==TRUE,`Jobs Titles Groups`:="General Doctor"]
dMaster[CW00isProf_Pharmacist==TRUE,`Jobs Titles Groups`:="Pharmacist"]
dMaster[CW00isProf_Midwife==TRUE,`Jobs Titles Groups`:="Midwife"]

dMaster[,from_richard:=TRUE]
dMaster[,richard_id1:=1:.N]
wor2[,from_system:=TRUE]
wor2[,richard_id2:=1:.N]

xf <- merge(
  dMaster,
  wor2,
  by.x=c("HR01","Jobs Titles Groups","IDH02.y"),
  by.y=c("ID No","Jobs Titles Groups","Facility"),
  all=T
  )
...
#idh02
if(length(na.omit(xf$richard_id1))>0){
  to_save <- dMaster[richard_id1 %in% na.omit(xf$richard_id1)]
  print(nrow(to_save))
  openxlsx::write.xlsx(
    to_save,
    fs::path(SHARED_TODAY,"crosscheck","work_missing_in_system.xlsx")
  )
}
if(length(na.omit(xf$richard_id2))>0){
  to_save <- wor2[richard_id2 %in% na.omit(xf$richard_id2)]
  print(nrow(to_save))
  openxlsx::write.xlsx(
    to_save,
    fs::path(SHARED_TODAY,"crosscheck","work_extra_in_system.xlsx")
  )
}
...
```{r, warning=FALSE}
dMaster <- merge(dWorkers, unique(dFacilities[,c("IDH00","facilitySector","IS06")]), by="IDH00",all.x=T)
nrow(dWorkers)
nrow(dMaster)
...
dMaster <- merge(dWorkers, unique(dLocation[,c("IDH03","region","district")]), by="IDH03",all.x=T)
nrow(dWorkers)
```
...
```{r, warning=FALSE}
dMaster <- merge(dWorkers, unique(dFacilities[,c("IDH00","IS06")]), by="IDH00",all.x=T)
nrow(dWorkers)
nrow(dMaster)
...
nrow(dMaster)
dMaster <- merge(dMaster, unique(dFacilities[,c("IDH00","facilitySector","IS06")]), by="IDH00", all.x=T)
nrow(dMaster)
...
```{r message=FALSE, warning=FALSE}
dMaster <- merge(dWorkers, unique(dFacilities[,c("IDH00","facilitySector")]), by="IDH00")
nrow(dWorkers)
nrow(dMaster)
...
```{r, warning=FALSE}
# todo
dMaster <- merge(dWorkers, unique(dFacilities[IS06==9,c("IDH00","facilitySector")]), by="IDH00")
nrow(dWorkers)
nrow(dMaster)
...
```{r, warning=FALSE}
dMaster <- merge(dWorkers, unique(dFacilities[IS06==4,c("IDH00","facilitySector")]), by="IDH00")
nrow(dWorkers)
nrow(dMaster)
unique(dMaster$facilitySector)
...
dMaster <- merge(dWorkers, unique(dFacilities[,c("IDH00","IS06")]), by="IDH00")
...
nrow(dMaster)
dMaster <- merge(dMaster, unique(dFacilities[,c("IDH00","facilitySector")]), by="IDH00", all.x=T)
nrow(dMaster)
...
## Healthcare workers with another Administrativesupportstaff job in the same facility
...
for(p in professionVars){
  for(i in c("isPalestine","isWB","isGaza")){
    cat("\n\n**********\n",p,i,"\n\n")
    d <- dMaster[get(i)==1]
    
    pCW00 <- sprintf("CW00%s",p)
    pCW06 <- sprintf("CW06%s",p)
    
    d <- d[
      (get(pCW00)==1 & CW06isProf_Administrativesupportstaff==1) |
      (get(pCW06)==1 & CW00isProf_Administrativesupportstaff==1)
      ,.(
      numerator=1
    ),by=.(
      HR01,
...
## Number of Nurses/Doctors in hospitals by District/Sector
...
x <- dMaster[IS06==9,.(
  nNurses=sum(isProf_Nurse, na.rm=T),
  nDoctors=sum(isProf_Doctor, na.rm=T)
),
keyby=.(facilitySector,region)]
print(x)
