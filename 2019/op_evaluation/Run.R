...
rmarkdown::render("AllGraphs.Rmd",output_dir=org::PROJ$SHARED_TODAY)
...
q <- q + scale_y_continuous("Enighetsskala",lim=c(0,1),breaks=seq(0,1,0.25),labels=c(1:5))
...
q <- q + scale_y_continuous("Enighetsskala")
q <- q + scale_x_discrete("")
q <- q + theme_gray(20)
q <- q + theme(legend.position="bottom")
q <- q + theme(axis.text.x = element_text(angle = 90, hjust = 1,vjust=0.5))
